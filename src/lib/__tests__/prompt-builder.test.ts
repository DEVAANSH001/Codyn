...
import { buildCodynPrompt, buildCodynVisualPrompt, formatHistoryText, resolveVisualOutputDecision } from "@/lib/prompt-builder";

describe("buildCodynPrompt", () => {
    const baseParams = {
    ...
    it("contains the user's question", () => {
        const result = buildCodynPrompt(baseParams);
    ...
    it("includes conversation history when provided", () => {
        const result = buildCodynPrompt({
            ...baseParams,
            historyText: "User: Hello\n\nCodyn: Hi there",
        });
        expect(result).toContain("User: Hello");
        expect(result).toContain("Codyn: Hi there");
    });
    ...
    it("includes Codyn persona instructions", () => {
        const result = buildCodynPrompt(baseParams);
        expect(result).toContain("Codyn");
    });
...
describe("buildCodynVisualPrompt", () => {
    const baseParams = {
    ...
    it("includes only routed diagram type prompt pack details", () => {
        const prompt = buildCodynVisualPrompt(baseParams);
    ...
    it("keeps compact visual-only contract and repo grounding", () => {
        ...
        expect(prompt).toContain("You are Codyn Visual Composer.");
...
describe("formatHistoryText", () => {
    ...
    it("formats a single model message", () => {
        ...
        expect(result).toContain("Codyn: Hi back!");
    });
    ...
    it("formats multi-turn conversation with separator", () => {
        ...
        expect(result).toContain("Codyn: A1");
