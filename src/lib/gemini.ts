...
import { buildCodynPrompt, buildCodynVisualPrompt, formatHistoryText } from "./prompt-builder";
...
function getModernGenAI(): GoogleGenAI {
  ...
  if (!apiKey) {
    throw new Error(
      "[Codyn] GEMINI_API_KEY environment variable is not set. " +
...
export async function answerWithContext(
  ...
  let prompt = useVisualOnlyPrompt
    ? buildCodynVisualPrompt({ question, context: enrichedContext, repoDetails, historyText })
    : buildCodynPrompt({ question, context: enrichedContext, repoDetails, historyText });
  const isProfileContext = repoDetails.repo === "profile";
  if (isProfileContext) {
    prompt += `\n\n[PROFILE TOOLS MODE]:
