    ...
    return (
        <main className="min-h-screen bg-black text-white p-6 md:p-12 overflow-x-hidden relative">
            <div className="fixed inset-0 z-0 pointer-events-none">
                <div className="absolute top-[-20%] left-[-10%] w-[80vw] max-w-[500px] h-[80vw] max-h-[500px] bg-cyan-600/10 rounded-full blur-[80px] md:blur-[128px]" />
            </div>
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    if (!valid) {
        return createSeoMetadata({
            title: `${owner}/${repo}`,
            description: `Analyze ${owner}/${repo} architecture, code quality, and security with Codyn Agentic CAG.`,
            canonical: `/repo/${owner}/${repo}`,
            ogImage: buildOgImageUrl("repo", { owner, repo }),
            ogTitle: `${owner}/${repo}`,
            ogDescription: `Analyze ${owner}/${repo} architecture, code quality, and security with Codyn Agentic CAG.`,
            noIndex: true,
        });
    }
        ...
        <main className="min-h-screen bg-black text-white p-6 md:p-12 overflow-x-hidden relative">
            ...
            <div className="max-w-5xl mx-auto relative z-10">
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                        <div className="w-full">
                            <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4">
                                ...
                                <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-cyan-500">{repoData.name}</span>
                            </h1>
                            {repoDescription && (
                ...
                <section className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-8 mb-12 backdrop-blur-sm">
                    ...
                    <p className="text-zinc-300 leading-relaxed mb-6">
                        This repository is indexed by Codyn. By analyzing <strong>{owner}/{repo}</strong> in our AI interface,
