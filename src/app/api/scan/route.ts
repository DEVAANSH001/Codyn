...
import { runTriage } from '@/lib/repo-analysis';
...
export async function POST(request: NextRequest) {
  ...
  catch (error) { return NextResponse.json({ error: error instanceof GitHubError ? error.message : 'Scan could not complete. No result has been saved. Retry or select another repository.' }, { status: error instanceof GitHubError ? error.status : 502 }); }
}
