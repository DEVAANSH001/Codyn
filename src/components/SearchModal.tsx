                                    ...
                                    <form onSubmit={handleSearch} className="space-y-5">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-500/50 outline-none transition-shadow"
                                            ...
                                            <div className="flex gap-2">
                                                <Tooltip text="Standard string search" className="flex-1 w-full">
                                                    <button
                                                        ...
                                                        disabled={loading}
                                                        className={`w-full py-2 rounded-xl text-xs font-semibold uppercase tracking-wider border transition-all ${searchType === 'text' ? 'bg-cyan-500/10 border-cyan-500/50 text-cyan-300 shadow-sm' : 'bg-zinc-800/50 border-white/10 text-zinc-400 hover:bg-zinc-700/50'} ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                                                    >
                                                        Text
                                                    </button>
                                                </Tooltip>
                                                <Tooltip text="Regular expression pattern matching" className="flex-1 w-full">
                                                    <button
                                                        ...
                                                        disabled={loading}
                                                        className={`w-full py-2 rounded-xl text-xs font-semibold uppercase tracking-wider border transition-all ${searchType === 'regex' ? 'bg-cyan-500/10 border-cyan-500/50 text-cyan-300 shadow-sm' : 'bg-zinc-800/50 border-white/10 text-zinc-400 hover:bg-zinc-700/50'} ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                                                    >
                                                        Regex
                                                    </button>
                                                </Tooltip>
                                                <Tooltip text="Abstract Syntax Tree matching (finds code structures)" className="flex-1 w-full">
                                                    <button
                                                        ...
                                                        disabled={loading}
                                                        className={`w-full py-2 rounded-xl text-xs font-semibold uppercase tracking-wider border transition-all ${searchType === 'ast' ? 'bg-cyan-500/10 border-cyan-500/50 text-cyan-300 shadow-sm' : 'bg-zinc-800/50 border-white/10 text-zinc-400 hover:bg-zinc-700/50'} ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                                                    >
                                                        AST
                                                    </button>
                                                </Tooltip>
                                        ...
                                        <div className="pt-2">
                                            <button
                                                ...
                                                className="w-full py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl font-semibold transition-colors disabled:opacity-50 flex items-center justify-center gap-2 shadow-md"
                                            >
