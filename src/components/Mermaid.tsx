...
function normalizeMermaidSvg(svgString: string, chartSource = ""): string {
    ...
    try {
        ...
        const existingStyle = doc.querySelector('style[data-codyn-font]');
        if (!existingStyle) {
            const styleEl = doc.createElement('style');
            styleEl.setAttribute('data-codyn-font', 'true');
...
function applyMindmapThemeOverrides(svgElement: SVGSVGElement): void {
    ...
    const centerFill = resolveThemeVar("--mindmap-center-fill", "#1769d2");
...
function applyGenericDiagramThemeOverrides(svgElement: SVGSVGElement): void {
    const lineColor = resolveThemeVar("--diagram-color-line", "#3D81E3");
...
export const Mermaid = ({ chart, isStreaming = false, rawCode }: MermaidProps) => {
    ...
    const fixedCacheKey = useMemo(() => `codyn:fixed-mermaid:${id}`, [id]);
