...
export function FileIcon({ filename, className }: FileIconProps) {
    ...
    switch (ext) {
        ...
        case 'tiff':
        case 'avif':
            return <FileImage className={cn("w-4 h-4 text-cyan-400 shrink-0 transition-colors", className)} />;
        case 'mp4':
        case 'mov':
        case 'avi':
        case 'webm':
...
export function FolderIcon({ name, isExpanded, className }: FolderIconProps) {
    ...
    const getIcon = () => {
        ...
        if (folderName === 'public' || folderName === 'assets' || folderName === 'images' || folderName === 'static') 
            return <FolderOpen className={cn(iconClass, "text-cyan-400")} />;
        if (folderName === 'components' || folderName === 'ui' || folderName === 'blocks') 
            return <FolderOpen className={cn(iconClass, "text-blue-400")} />;
